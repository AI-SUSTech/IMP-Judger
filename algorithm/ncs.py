...
def ncs(o, A, M, a, alpha, b, lu, problem):
    ...
    sp = np.tile(struct('x', np.zeros(D,mu),'fit', np.zeros(1,mu),'mean', np.zeros(D,1),'cov', np.zeros(D,1)), (_lambda, 1))
    ...
    while time <= totalTime:
        ...
        while FES <MAXFES:
            ...
            for i in range(1 ,_lambda):
                ...
                deltaMean_f = np.multiply(invCov_i, np.mean(np.multiply(difXtoMean, np.tile(utility,D,1)), 2)) # w.r.t. mean vector
                deltaCov_f =  np.divide(np.multiply(np.power(invCov_i, 2), np.mean(np.multiply(np.power(difXtoMean, 2), np.tile(utility,D,1)),2)), 2) # w.r.t. covariance matrix
                ...
                meanFisher = np.multiply(np.power(invCov_i, 2), np.mean(np.power(difXtoMean, 2),2)) # w.r.t. mean vector
                covFisher = np.divide(np.mean(np.power((np.power(np.multiply(np.tile(np.power(invCov_i, 2),1,mu), difXtoMean), 2)-np.tile(invCov_i,1,mu)), 2),2), 4) # w.r.t. covariance matrix
               
                # Update the probilistic model of the search process
                sp[i].mean = np.multiply(np.multiply(sp[i].mean+np.divide(1, meanFisher), (deltaMean_f+np.multiply(deltaMean_d, phi_init))), eta_m) # w.r.t. mean vector
                sp[i].cov = np.multiply(np.multiply(sp[i].cov+np.divide(1, covFisher), (deltaCov_f+np.multiply(deltaCov_d, phi_init))), eta_c) # w.r.t. covariance matrix
